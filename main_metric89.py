...
try:
    YOLO_MODEL = YOLO("./weights/gs25_yolov11_8_8.onnx", task="detect")
    ...
    def process_frame(self, frame):
        ...
        for track in tracks:
            ...
            if is_box_in_zone(box, zone_coords_3, 0.1):
                ...
                if is_box_in_zone(box, zone_coords_4, 0.07) or self.rtsp_url == "rtsp://cxview:gs252525@42.1.117.16:554/Streaming/Channels/801":
                    stopped_2 = False
        ...
        for t_id, info in self.track_person.items():
            if self.current_frame - info["last_seen_frame"] > 200:
                if info["stopped_1"] and info["total_time_2"] > 0 and not info["reid"] and self.send_api:
                    if info["total_time_1"] < 10:
                        info["total_time_1"] = info["total_time_1"] + 10
                    if info["total_time_2"] < 20:
                        info["total_time_2"] = info["total_time_2"] + 20
...
def parse_args():
    ...
    parser.add_argument('--store_id', type=str, default='vn300',
                        help='Store ID (the key in camera_configs)')
